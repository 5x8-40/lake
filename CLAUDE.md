...
2. **按主题定位文档**：
 ...
 - **模型定义(Transformers)**:Qwen3 `nn.Module`/HF config/decoder/model/causal-lm 分层 → `docs/research/transformers/overview.md`
   ...
   - **NVIDIA CMX**（公布的目标栈：GPU + Dynamo + NIXL + DOCA Memos + 双端 BF4 + STX；当前 Dynamo 无一等 G3.5、NIXL 插件仍是 open PR；VAST 当前 G3 实验 ≠ CMX benchmark）→ `docs/research/nvidia-cmx.md`（证据分级、布局字节与 offered-load 计算器 `tools/cmx-sim/`）
