...
2. **按主题定位文档**：
 ...
 - **跨节点 KV 共享 / KVBM 继任者(KVCR)**:引擎进程内二级存储 + router hint P2P + NIXL + Guard 容错 + 策略可插拔;与 KVBM 的区别与重做原因 → `docs/research/kvcr/overview.md`
   ...
   - **模型级路由与 APC 亲和调度**:OpenAI/Databricks/OpenRouter/OpenSquilla 等模型路由产品、RouteLLM 等论文、Anthropic prompt caching 经验、production-stack/SGLang/AIBrix/llm-d/Kthena 的缓存亲和与会话调度、对实例级 Router(Dynamo/lake)的借鉴 → `docs/research/model-routing.md`
