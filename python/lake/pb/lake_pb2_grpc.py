...
class ControlPlaneServiceStub:
    ...
    原则:KV 字节走 RDMA 旁路,proto 只定义控制 msg(查询/命令/元数据/完成回执)。
    worker ↔ in-process agent 同进程走 FFI(PyO3),不进 proto。

    3 个 service 按通信边划分:
    ControlPlaneService — Router+agent ↔ 存储控制面(控制面进程实现)
    AgentService        — Router/控制面 → agent(agent 进程实现)
    TransferService     — 节点间传输控制信令(字节旁路;**agent / KV Node agent 实现,非控制面**)
