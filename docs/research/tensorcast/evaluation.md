...
**lake 注**:这正是 lake 权重缓存(F2)要复现的场景。"cold 也快 60×"的关键不在缓存而在**分片 view + 多路读 + H2D 流水线**——lake 的 tiered-store pipeline 已具备同构路径;"warm < 1s"对应 lake 的 L0 预放置(方案 Z:池主动把权重/前缀 KV 预置到 HBM)。注意基线是**分布式文件系统**——对象存储/共享盘慢是放大的前提,本地 SSD 上差距会收窄(论文图 5/6 有三存储对比,正文只引 JFS 数字)。
