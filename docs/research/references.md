...
- **NVIDIA CMX** (Inference Context Memory Storage / ICMS): Vera Rubin 上的 G3.5 上下文层。核心是 **DOCA Memos 双端 KV I/O**（计算节点 BF4 模拟 KV 设备 + STX 柜 BF4 把最后一跳落到 SSD block），不是「一块更快的企业盘」。公开材料分析见 [`nvidia-cmx.md`](nvidia-cmx.md)；定量仿真见 [`../../tools/cmx-sim/`](../../tools/cmx-sim/)。
