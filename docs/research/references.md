...
- **NVIDIA CMX** (Context Memory Storage；原 Inference Context Memory Storage / ICMS): NVIDIA 公布的完整 KV cache 存储**目标栈** = GPU + Dynamo + NIXL + DOCA Memos + 双端 BF4 + STX；它不是与 lake 平级的软件，lake 只会把相关共享 flash 路径作为候选 L2 backend。当前公开 Dynamo 没有一等 G3.5，NIXL Memos 后端仍是 open PR。VAST 给出 CNode/DNode/DASE 和 10k 用户容量方法，但其 20× 结果是当前 G3 路径，不是 CMX benchmark。文档与计算器分开 logical/entry payload、engine page、custom wire、external/GPU-saturated offered load、P→D handoff、TP/CP/副本和经济分母；另收录匿名 Cursor trace、provider cache 留存及 100 GB File Library 澄清。后续见 lake [#22](https://gitlab.com/BeeBreeze/lake/-/issues/22) 与独立项目孵化 [#23](https://gitlab.com/BeeBreeze/lake/-/issues/23)。证据分级见 [`nvidia-cmx.md`](nvidia-cmx.md)；分析计算器见 [`../../tools/cmx-sim/`](../../tools/cmx-sim/)。
