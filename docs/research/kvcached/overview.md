...
收益的账本：**权重是常驻成本，KV 是变动成本**——弹性机制只省 KV，权重省不了（除非共享）。**默认答案是扩缩容**：头部模型流量大且稳，一实例一卡 + 扩缩容，权重不重复、方案简单。超卖/混部只处理扩缩容够不着的三块：

1. **实例内部闲置的 KV**（分配满 ≠ 使用满）：静态预留让低 QPS / 波谷实例的 KV 大部分闲置，且进程私有、别人借不走；扩缩容调的是实例数，够不着实例内部。弹性 KV 让闲置页在实例/模型间流转——各模型峰值不同时到来，共享后按同时刻总需求供给，赚峰值差值。
2. **不足一卡的形态与模型**（单卡碎片）：某形态/模型的流量喂不满一张卡时（投机分流里的延迟形态、长尾小模型），扩缩容以整卡为单位，填不满剩余部分；混部把多个装上一卡。同模型多实例若确有必要（上段五个例外），权重可经 CUDA IPC 共享（`cuMemCreate` 导出句柄、他进程 import 映射；TensorCast daemon 模式、ServerlessLLM 已验证，见 [`../tensorcast/architecture.md`](../tensorcast/architecture.md)），即「1 份权重 + N 个弹性 KV 池」，边际成本只剩 CUDA context + 图 + workspace；代价是物理页生命周期跨进程耦合（需租约或所有权归池）、sleep 粒度变粗。
3. **秒级突发与冷模型挂载**（时间常数）：扩缩容是十秒~分钟级（权重加载 + 图捕获 + warmup），波峰来了实例还没起来，只能拒或排队；冷模型不能缩到 0（冷启动分钟级），挂着就占一份权重。sleep + 弹性 KV 让一张卡挂几十个冷模型、同时只醒几个（serverless 多模型的核心）。

两个特例：

- **LoRA（收益最大化）**：base 权重共享，每模型只多一个 adapter；multi-LoRA 在单实例内按请求选 adapter（vLLM/SGLang 原生支持），连多实例都不需要。
- **kvcached 反哺扩缩容**：特性 3（零物理分配冷启动）+ sleep 组合，把扩缩容的时间常数压下来——选扩缩容路线时 kvcached 同样有用。

**边界**：流量平稳、模型少、冷启动可接受的场景，纯扩缩容更简单，不必引入 VMM。
