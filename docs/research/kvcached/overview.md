...
收益的账本：**权重是常驻成本，KV 是变动成本**——弹性机制只省 KV，权重省不了（除非共享）。按场景分三种：

**① 同模型多实例**：权重可通过 CUDA IPC 共享——`cuMemCreate` 导出句柄、他进程 import 映射，多实例读同一份物理权重（TensorCast daemon 模式、ServerlessLLM 已验证，见 [`../tensorcast/architecture.md`](../tensorcast/architecture.md)），配合 KV 弹性即「1 份权重 + N 个弹性 KV 池」。
- 代价：物理页生命周期跨进程耦合（拥有者退出/sleep 会抽走，需租约或所有权归池）；sleep 粒度变粗（最后一个清醒实例才能睡权重）。
- 共享后多实例的边际成本只剩 CUDA context + 图 + workspace，「要不要多实例」回到上段五个例外。

**② 不同模型混部**：权重 N 份省不掉，收益全在 KV 与睡眠上：
- KV 削峰错谷——各模型峰值不同时到来，共享后按同时刻总需求供给，赚峰值差值；
- 挂载集 ≫ 活跃集——闲置模型权重 sleep 到 DRAM 或卸载，一张卡可挂几十冷模型、同时只醒几个（serverless 多模型的核心）；
- 长尾聚合——各自 QPS 低的模型混部摊薄。
- 边界：所有模型常驻且全热时，收益只剩 KV 弹性。

**③ LoRA（收益最大化的特例）**：base 权重共享，每个模型只多一个 adapter；multi-LoRA 在单实例内按请求选 adapter（vLLM/SGLang 原生支持），连多实例都不需要。

**与扩缩容的关系**：两者正交——扩缩容调节卡数，混部提高单卡密度。生产上的分工：**头部模型（流量大且稳）一实例一卡 + 扩缩容**，权重不重复、HBM 跑满、方案简单；**长尾模型（几十个、各自 QPS 低）混部 + 超卖**，填碎片、吸收突发。注意「扩缩容把 HBM 用满」的错觉：静态预留让 HBM **分配满**而非**使用满**——低 QPS 或波谷时，实例预留的 KV 大部分闲置，但进程私有、别人借不走；扩缩容调的是实例数，够不着实例内部的闲置。扩缩容替代不了混部，因为：
- 时间常数不匹配——扩缩容是十秒~分钟级（权重加载 + 图捕获 + warmup），流量突发是秒级，波峰来了实例还没起来，只能拒或排队；
- 单卡碎片——模型吃不下整卡时（如 50G/80G），扩缩容以卡为单位，填不满剩余部分；
- 前提是有闲卡——容量受限的集群里，混部提高的是存量卡的密度。

反过来，kvcached 也让扩缩容更快（特性 3 零物理分配冷启动 + sleep 组合）。**边界：流量平稳、模型少、冷启动可接受的场景，纯扩缩容更简单，不必引入 VMM**。
...
6. **权重 VMM 化 / MoE 专家懒加载**。kvcached 只虚拟化 KV；同一机制可用于权重：MoE 专家 VA 预留，热专家常驻物理页，冷专家释放（字节在 DRAM/SSD，用时换入），单卡逻辑容量超物理容量。与 TensorCast 权重 artifact 化、lake Weight Cache 同方向。风险也最大：专家切换在 decode 路径上，换入延迟直接进 ITL，必须按路由分布做预测性预取。
