...
| **轻量分类器** | 小模型/BERT/LightGBM 给请求打难度或类型标签,标签映射到模型档位 | 两个子类:(a) **推理时现打**——用 prompt 让便宜小模型按预设字段输出语义标签,不训练(Databricks);(b) **训练小分类器**——标签来自公开/合成任务数据集(vLLM-SR 的 ModernBERT)或自家 harness 执行日志回流(OpenSquilla 的"数据飞轮") | Databricks、OpenSquilla、vLLM Semantic Router、HybridLLM |
