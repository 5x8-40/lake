...
两层路由(模型级选模型/API、实例级选 worker)的厂商产品、开源实现与学术调研,以及对本 Router 的借鉴方向,见 [`../model-routing.md`](../model-routing.md)。
...
- **双环控制**:Planner 里有两个扩缩容回路,各管一种时间尺度——
  ...
  | **输入** | Prometheus 流量指标(请求数/ISL/OSL),经预测器外推下一时段 | 引擎每次前向的实时指标(FPM,见下) |
  ...
  其中 **FPM(ForwardPassMetrics,前向指标)** 是引擎每跑完一次前向迭代就发一条的结构化指标:本次迭代耗时(`wall_time`)、调度进去的 prefill token 总数、decode KV token 总数、排队请求数等,经事件面(默认 ZMQ)发布;空闲心跳(耗时为 0)会被跳过。快环订阅它做在线回归、估当前 TTFT/ITL;可观测性层也会把 FPM 旁路落盘(见「可观测性」节)。
