...
> 旁证:[kvcached](../kvcached/overview.md)(GPU 虚拟内存弹性 KV)与 KVCR 的边界正好互补——KVCR 不碰 GPU、引擎给指针;kvcached 只碰 GPU 页映射、不管 KV 语义。两者组合(kvcached 管 HBM 弹性 + KVCR 管下层卸载)的冲突面取决于传输端点:若 NIXL 注册的是 KVCR 自有 DRAM(主层中转),与 GPU 页重映射无关;只有 RDMA 端点是 GPU 显存(G1→G1 直传)时才冲突,候选解法是 RDMA ODP(见该文 §4.3)。
