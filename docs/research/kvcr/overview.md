...
> 旁证:[kvcached](../kvcached/overview.md)(GPU 虚拟内存弹性 KV)与 KVCR 的边界正好互补——KVCR 不碰 GPU、引擎给指针;kvcached 只碰 GPU 页映射、不管 KV 语义。两者组合(kvcached 管 HBM 弹性 + KVCR 管下层卸载)的冲突面取决于传输端点:若 NIXL 注册的是 KVCR 自有 DRAM(主层中转),与 GPU 页重映射无关;只有 RDMA 端点是 GPU 显存(G1→G1 直传)时才冲突,候选解法是 RDMA ODP(见该文「想象空间」节)。
...
"全局视图集中在 router,查询和更新两路压力是否会成为瓶颈"是这个架构的显然疑问,KVCR 设计文档的 Scalability and Locality 节就是回应它:**集群里只有 router 的库存表随集群规模增长;KVCR 上报是部分的、最终一致的,够用即可,不追求完整强一致**。具体手段(Dynamo `lib/kv-router` 实现,自称 FlashIndexer):

- **读写分离的索引**:路由查询是 radix 树(前缀树)上的前缀匹配,开销正比于请求前缀长度而非库存总量;`thread_pool.rs` 的线程池索引器让读请求就地执行、不写队列排队。官方给出的量级:**事件+请求合计 1000 万+/秒,p99 延迟 <10µs**(`indexer/README.md`)。
- **更新侧减负**:KVCR 只上报受影响的 key、可批量;`pruning.rs` 用 TTL 分桶做近似过期,不要求精确的驱逐事件;`cuckoo/` 用布谷鸟过滤器(概率型成员判断)压缩下层库存的内存占用。
- **分区**:`identity.rs` 定义了 routing partition(按 model/group 划分),库存按分区拆开,不存在单一全局结构;还有数据中心本地(DC-local)的事件池。
- **去中心化出口**:`local.rs` 支持 worker 侧持有本地索引副本(环形事件缓冲),路由决策可以下沉。

这个架构能成立的根本原因是:**视图不准不会出错,只会次优**。`FETCHABLE` 的块拉取失败就回退重算,路由选错节点只是命中率损失——所以视图可以用新鲜度换规模。真正的瓶颈不在 CPU,而在内存(唯一 block 数 × 每条元数据)和事件总线的扇出;视图过旧表现为命中率下降,是质量问题而非可用性问题。
