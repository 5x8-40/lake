...
#
# Usage (from host):
#   bash scripts/ascend/start_dynamo_va_native.sh                 # MODE=agg (colocated)
#   MODE=pd bash scripts/ascend/start_dynamo_va_native.sh         # P/D disaggregated (4+4 NPUs)
...
#
# P/D mode layout (8 NPU defaults, override for other shapes):
...
MODE=${MODE:-agg}   # agg = colocated prefill+decode | pd = disaggregated P/D
TP=${TP:-4}
DP=${DP:-2}
# P/D layout (only used when MODE=pd)
P_NPU=${P_NPU:-0,1,2,3}
P_TP=${P_TP:-4}
P_DP=${P_DP:-1}
D_NPU=${D_NPU:-4,5,6,7}
...
if [ "$MODE" != "agg" ] && [ "$MODE" != "pd" ]; then
  echo "MODE must be agg or pd, got: $MODE" >&2
  exit 2
fi
...
if [ "$MODE" = "pd" ]; then
  ...
  echo "started inside $NAME (FE :$PORT, MODE=agg, worker TP${TP}xDP${DP}, model=$SERVED_NAME, discovery=$DISCOVERY, mooncake=$MOONCAKE_RPC_PORT)"
fi
if [ "$DISCOVERY" = "etcd" ]; then
...
if [ "$ok" = "1" ]; then
  ...
  if [ "$MODE" = "pd" ]; then
    ...
    echo "--- worker log ---"
    tail -40 "$LOGDIR/worker.log" || true
  fi
  echo "--- mooncake log ---"
