...
本文蒸馏自 2026-09-17 的一次外部讨论(chat-export),并已对照 lake 已有调研修正其中的过时与错误点(见下"修正")。
...
## 修正(讨论中的过时/错误点,以 lake 调研为准)

1. **KVBM 已 sunset**:讨论中"控制面(Router/KVBM)直接复用"的说法过时——上游 KVBM 2026-07 被官方废弃(DEP #11673),继任者是 **KVCR**(引擎进程内二级存储 + router hint 驱动 P2P,见 [`../../research/kvcr/overview.md`](../../research/kvcr/overview.md))。存储/传输适配必须对标 KVCR 的接口与后端位,不是 KVBM。
2. **"Dynamo Router 无物理拓扑感知"存疑**:Run:ai 集成恰是 topology-aware 调度;昇腾 UB 域拓扑感知是否真是增量,需在适配设计时重新核实上游 Router 的拓扑能力后再断言。
3. 讨论中的 Dynamo 用户名单证据质量参差(Kimi recipes 属实,见上游 `docs/fern/pages/recipes/model-recipes/kimi-k3.mdx`;其余链接未逐一核实),不作为决策依据。
