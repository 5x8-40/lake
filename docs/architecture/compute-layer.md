...
> 对照全集见 [`../research/scheduler-worker-interface.md`](../research/vllm_vs_sglang/scheduler-worker-interface.md)。定稿原则：**调度几何与信封外形偏 vLLM**（`num_computed_tokens` + 本步 `num_scheduled_tokens` + 增量 new/cached + `execute_model(SchedulerOutput)`）；**不**用 SGLang 式 Prefill/Extend/Decode/PREBUILT **分相状态机**驱动执行；`ForwardMode` 仅为由几何派生的批标签（图/日志）；DP 字段（`global_num_tokens*`）仍可挂 Scheduler；**KV 句柄两边都不照搬**。
...
> 对照：[`../research/vllm/compute.md`](../research/vllm/compute.md)、[`../research/scheduler-worker-interface.md`](../research/vllm_vs_sglang/scheduler-worker-interface.md)。  
...
**C13 状态（2026-07-27）**：已落 `GrammarOutput` / `SamplingParams.structured_output` 占位，`ModelRunner.sample_tokens()` 可消费 bool-list bitmask 或 defer 本步 sample；`NodeScheduler.schedule()` 会把 structured 请求标进 `SchedulerOutput.grammar_output`，`spec + structured + overlap` 且存在未处理结果时强制 drain，对齐 SGLang `need_grammar_sync`。默认 Qwen3 dummy backend 也先产 dummy logits 再进入统一 `sample_tokens()`，因此 bitmask/defer 等 hook 不会被默认路径绕过。同时修正 TARGET_VERIFY 在剩余生成预算截短 draft 时的 query/write slot 对齐：调度只传本步可验证 draft，runner 防御性按 `n-1` 截断。真 xgrammar/llguidance FSM、packed bitmask tensor、H2D copy stream、grammar-aware speculative rejection 后置；其中 verify 前沿预计算能覆盖什么、为何不能仅凭“按接受长度选状态”消除 host commit 依赖，见 [`../research/guided-decoding.md`](../research/vllm_vs_sglang/guided-decoding.md)「draft tree 的预计算」和「GrammarFrontier」。
...
| D7 | **Sampling / structured output 挂载点** | 状态归属已有 research;engine 内 `sample/` 与 grammar bitmask 的 step 序(相对 `execute_model`/`sample_tokens`)未钉 | 见 [`../research/sampling-params.md`](../research/vllm_vs_sglang/sampling-params.md)、[`../research/guided-decoding.md`](../research/vllm_vs_sglang/guided-decoding.md) |
